...
workerFile<-"AllMT_task/Results/MT_2_4_3_5_split_worker_Hits.csv"
total=1000
...
newcorr<-cbind(Workers = rownames(corr), corr) 
newcorr[is.na(newcorr)] <- " "
write.csv(newcorr,OutputFile, row.names=F)
colnm<-colnames(corr)
colnm
workerdata<-read.table("AllMT_task/Results/MT_2_4_3_5_split_worker.txt", header=T, sep="\t")
newworker<-cbind(Workers = rownames(workerdata), workerdata) 

write.csv(newworker,workerFile,row.names=F)
